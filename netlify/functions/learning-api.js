...
exports.handler=async(event)=>{
 ...
 if(!p&&!isAdmin){if(!deviceId)return json(401,{error:'Please create an account to continue.',code:'registration_required'});const guestTrialKey='__one_section_guest_trial__';const prior=await arr('/rest/v1/tool_usage?device_id=eq.'+encodeURIComponent(deviceId)+'&tool_key=eq.'+encodeURIComponent(guestTrialKey)+'&select=id&limit=1').catch(()=>[]);if(prior[0])return json(401,{error:'Your one-section free preview is complete. Please create an account or log in to continue.',code:'registration_required'});await sf('/rest/v1/tool_usage',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({device_id:deviceId,tool_key:guestTrialKey})}).catch(()=>{})}
 ...
 const questionCount=Math.max(1,Math.min(10,Number(b.question_count)||5));const examType=clean(b.exam_type,20).toLowerCase();
 ...
 try{
  ...
  const prompt=['You are Salone Class Room, an educational assistant for Sierra Leone.',role,depth,curriculumRule,'Task: '+task+'.',taskRules[task]||'Explain accurately and use examples where they improve understanding.','Level: '+level+'.','Class: '+classLevel+'.','Subject: '+subject+'.','Learner/teacher request: '+topic+'.',task==='quiz'?'Required number of questions: '+questionCount+'.':'',task==='quiz'&&excludedQuestions.length?'Do NOT repeat any of these earlier questions: '+excludedQuestions.join(' || '):'',curriculumGrounding?'Approved/local curriculum database context:\n'+curriculumGrounding:'No matching curriculum database context was retrieved for this request.',context?'Additional form information: '+context+'.':'','Generation request nonce: '+clean(b.request_nonce,80)+'. Produce a fresh response rather than repeating a previous response when the user generates again.','Do not fabricate citations. Use clean headings and paragraphs where helpful. Do not output raw markdown bold asterisks.'].filter(Boolean).join('\n');

  const callGroq=async()=>{
   if(!GROQ)throw new Error('Groq key unavailable');
   const body={model:groqModel,messages:[{role:'user',content:prompt}],temperature:task==='quiz'?0.75:0.45,max_tokens:task==='quiz'?3000:(task==='teacher'?2600:(more?2600:1600))};
  ...
  const callDeepSeek=async()=>{
   ...
   const body={model:deepseekModel,messages:[{role:'user',content:prompt}],thinking:{type:'disabled'},temperature:task==='quiz'?0.75:0.45,max_tokens:task==='quiz'?3000:(task==='teacher'?2600:(more?2600:1600))};
   if(task==='quiz')body.response_format={type:'json_object'};
  ...
  if(task==='quiz'){
   ...
   if(questions.length!==questionCount)return json(502,{error:'The question service did not return all requested questions. Please try again.'});
