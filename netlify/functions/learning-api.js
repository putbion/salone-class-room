...
exports.handler=async(event)=>{
 ...
 const questionCount=Math.max(1,Math.min((task==='teacher_exam'||task==='teacher_exam_objective'||task==='teacher_exam_theory')&&examType==='objective'?10:5,Number(b.question_count)||5));
 ...
 const allowedTasks=new Set(['tutor','assignment','review','topics','proposal','dissertation','defense','teacher','teacher_exam','teacher_exam_objective','teacher_exam_theory','quiz','lecturer']);if(!allowedTasks.has(task))return json(400,{error:'Unsupported learning task.'});
 ...
 try{
  ...
  const callGroq=async()=>{
   ...
   if(task==='quiz'||task==='teacher_exam_objective')body.response_format={type:'json_object'};
  ...
  if(task==='teacher_exam_objective'){
   ...
   if(questions.length!==10)return json(502,{error:'The objective question service did not return exactly 10 multiple-choice questions. Please tap Generate Questions again.'});
   ...
   return json(200,{source:'ai',provider:result.provider,model:result.model,admin_access:isAdmin,usage:{input_tokens:inputTokens,output_tokens:outputTokens},answer});
  }
  if(task==='quiz'){
