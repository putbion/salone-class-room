...
exports.handler=async(event)=>{
 ...
 try{
  ...
  const callGroq=async()=>{
   ...
   const body={model:groqModel,messages:[{role:'user',content:prompt}],temperature:task==='quiz'?0.75:0.45,max_tokens:task==='quiz'?3000:3500};
  ...
  const callDeepSeek=async()=>{
   ...
   const body={model:deepseekModel,messages:[{role:'user',content:prompt}],thinking:{type:'disabled'},temperature:task==='quiz'?0.75:0.45,max_tokens:task==='quiz'?3000:3500};
   if(task==='quiz')body.response_format={type:'json_object'};
  ...
  if(task==='quiz'){
   ...
   return json(200,{source:'ai',provider:result.provider,model:result.model,usage:{input_tokens:inputTokens,output_tokens:outputTokens},questions});
  }
  return json(200,{source:'ai',provider:result.provider,model:result.model,usage:{input_tokens:inputTokens,output_tokens:outputTokens},answer:text});
