...
exports.handler=async(event)=>{
 ...
 if(event.httpMethod==='POST'){
  ...
  if(b.action==='save_announcement'){
   if(b.active)await sf('/rest/v1/announcements?active=eq.true'+(b.id?'&id=neq.'+encodeURIComponent(b.id):''),{method:'PATCH',body:'{"active":false}'});
   ...
   const r=b.id?await sf('/rest/v1/announcements?id=eq.'+encodeURIComponent(b.id),{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify(row)}):await sf('/rest/v1/announcements',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify(row)});return r.ok?json(200,{ok:true}):json(500,{error:'Unable to save announcement.'});
  }
  if(b.action==='delete_announcement'){
   const r=await sf('/rest/v1/announcements?id=eq.'+encodeURIComponent(b.id),{method:'DELETE'});return r.ok?json(200,{ok:true}):json(500,{error:'Unable to delete announcement.'});
  }
 ...
 const active30=profiles.filter(p=>p.last_seen_at&&now-new Date(p.last_seen_at).getTime()<=d30).length,inactive30=profiles.length-active30;
 const d1=86400000,d7=7*d1;const aiWindow=(ms)=>aiUsage.filter(x=>x.created_at&&now-new Date(x.created_at).getTime()<=ms);
 const aiSummary={day:aiWindow(d1).length,week:aiWindow(d7).length,month:aiWindow(d30).length};
 const aiByProfile={};for(const x of aiUsage){const id=x.profile_id||('device:'+String(x.device_id||'unknown'));if(!aiByProfile[id])aiByProfile[id]={day:0,week:0,month:0};const age=now-new Date(x.created_at).getTime();if(age<=d30)aiByProfile[id].month++;if(age<=d7)aiByProfile[id].week++;if(age<=d1)aiByProfile[id].day++;}

 const pays=payments.map(p=>({...p,full_name:p.user_profiles?.full_name,category:p.user_profiles?.category}));
 return json(200,{features,admin_identity:{email:(au.email||'').toLowerCase(),role:access.role},admins,reset_requests:resetRequests,summary:{registered_users:profiles.length,new_users_30d:profiles.filter(u=>new Date(u.created_at).getTime()>=now-d30).length,events_30d:e30.length,new_messages:messages.filter(x=>(x.status||'new')==='new').length,online_now:active5,active_today:activeToday,active_users_30d:active30,inactive_users_30d:inactive30,ai_calls_30d:aiUsage.filter(x=>new Date(x.created_at).getTime()>=now-d30&&x.source==='ai').length},categories:c,metrics:m,users,events,messages,profiles,payments:pays,ai_usage:aiUsage,platform_settings:settings[0]||{},announcement:anns.find(x=>x.active)||anns[0]||null,announcements:anns,ai_usage_summary:aiSummary,ai_usage_by_profile:aiByProfile,system:{supabase:true,events_table:true,messages_table:true}});
}
;
