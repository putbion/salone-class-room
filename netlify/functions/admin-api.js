...
exports.handler=async(event)=>{
 ...
 const [events,messages,profiles,payments,settings,anns,aiUsage,admins,resetRequests,features]=await Promise.all([arr('/rest/v1/app_events?select=*&order=created_at.desc&limit=500'),arr('/rest/v1/support_messages?select=*&order=created_at.desc&limit=300'),arr('/rest/v1/user_profiles?select=*&order=created_at.desc&limit=1000'),arr('/rest/v1/payment_requests?select=*,user_profiles(full_name,category)&order=created_at.desc&limit=500'),arr('/rest/v1/platform_settings?id=eq.1&select=*'),arr('/rest/v1/announcements?select=*&order=created_at.desc&limit=1'),arr('/rest/v1/ai_usage_log?select=*&order=created_at.desc&limit=2000'),arr('/rest/v1/admin_users?select=*&order=created_at.asc'),arr('/rest/v1/password_reset_requests?select=*,user_profiles(full_name,contact,email,phone)&order=created_at.desc&limit=200'),arr('/rest/v1/platform_features?select=*&order=display_order.asc,feature_name.asc')]);
 const normalizedProfiles=profiles.map(p=>{const email=clean(p.email,254),phone=clean(p.phone,80),fallback=clean(p.contact,254);return {...p,email,phone,contact:fallback||(email||phone||''),contact_display:[email&&('Email: '+email),phone&&('Phone: '+phone)].filter(Boolean).join(' | ')||fallback||'No contact'}});
 const now=Date.now(),d30=2592000000,e30=events.filter(e=>{const t=new Date(e.created_at).getTime();return Number.isFinite(t)&&t>=now-d30}),contains=(e,w)=>w.some(x=>(((e.event_type||'')+' '+(e.section||'')+' '+(e.detail||'')).toLowerCase()).includes(x));
 ...
 const lastActivity=p=>{const vals=[p.last_seen_at,p.last_login_at].map(v=>v?new Date(v).getTime():NaN).filter(Number.isFinite);return vals.length?Math.max(...vals):NaN};
 const active5=normalizedProfiles.filter(p=>{const t=lastActivity(p);return Number.isFinite(t)&&now-t>=0&&now-t<=300000}).length,activeToday=normalizedProfiles.filter(p=>{const t=lastActivity(p);return Number.isFinite(t)&&now-t>=0&&now-t<=86400000}).length;
 const pays=payments.map(p=>({...p,full_name:p.user_profiles?.full_name,category:p.user_profiles?.category}));
 return json(200,{features,admin_identity:{email:(au.email||'').toLowerCase(),role:access.role},admins,reset_requests:resetRequests,summary:{registered_users:normalizedProfiles.length,new_users_30d:normalizedProfiles.filter(u=>{const t=new Date(u.created_at).getTime();return Number.isFinite(t)&&t>=now-d30}).length,events_30d:e30.length,new_messages:messages.filter(x=>(x.status||'new')==='new').length,online_now:active5,active_today:activeToday,ai_calls_30d:aiUsage.filter(x=>{const t=new Date(x.created_at).getTime();return Number.isFinite(t)&&t>=now-d30}).length},categories:c,metrics:m,users,events,messages,profiles:normalizedProfiles,payments:pays,ai_usage:aiUsage,platform_settings:settings[0]||{},announcement:anns[0]||null,system:{supabase:true,events_table:true,messages_table:true}});
}
;
