...
exports.handler=async(event)=>{
 ...
 if(event.httpMethod==='POST'&&b.action==='admin_login'){
   const email=clean(b.email,180).toLowerCase(),password=String(b.password||'');const access=await adminAllowed(email);if(!access.allowed)return json(403,{error:'This email is not authorized as a Salone Class Room administrator.'});
 ...
 if(event.httpMethod==='POST'){
  if(b.action==='admin_add'){
   ...
   const email=clean(b.email,180).toLowerCase(),password=String(b.password||''),role='assistant';if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)||password.length<8)return json(400,{error:'Enter a valid email and a temporary password of at least 8 characters.'});
