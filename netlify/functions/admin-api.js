...
exports.handler=async(event)=>{
 ...
 if(event.httpMethod==='POST'){
  ...
  if(b.action==='update_announcement'){
   if(!b.id)return json(400,{error:'Announcement ID is required.'});if(b.active)await sf('/rest/v1/announcements?active=eq.true&id=neq.'+encodeURIComponent(b.id),{method:'PATCH',body:'{"active":false}'});const row={message:clean(b.message,1000),active:!!b.active,starts_at:b.starts_at||null,ends_at:b.ends_at||null,target_category:clean(b.target_category,30)||null};const r=await sf('/rest/v1/announcements?id=eq.'+encodeURIComponent(b.id),{method:'PATCH',body:JSON.stringify(row)});return r.ok?json(200,{ok:true}):json(500,{error:'Unable to update announcement.'});
  }
  if(b.action==='delete_announcement'){
   if(!b.id)return json(400,{error:'Announcement ID is required.'});const r=await sf('/rest/v1/announcements?id=eq.'+encodeURIComponent(b.id),{method:'DELETE'});return r.ok?json(200,{ok:true}):json(500,{error:'Unable to delete announcement.'});
  }
